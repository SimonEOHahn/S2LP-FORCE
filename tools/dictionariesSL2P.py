...
def define_input_resolution():
    RESOLUTION_OPTIONS = {  
        ...
        'S2_FORCE': 20,      # Sentinel 2 FORCE Tile (Set to use 20m resolution and 20m network)
        'S2_SINGLE_TIF': 10  # Custom single-TIF 10m product (no offset)
    }
...
def make_collection_options(fc):  
    COLLECTION_OPTIONS = {
        ...
        # Sentinel 2 FORCE (Aligned to S2_SR / 20m Network)
        'S2_FORCE': {
        "name": 'S2_FORCE',
        "description": 'Sentinel 2 FORCE Tile (20m Network)',
        ...
        "Network_Ind": fc.s2_createFeatureCollection_Network_Ind(),      
        "numVariables": 7,
        "exportRes": 20, # Output at 20m resolution
        },

        # S2_SINGLE_TIF (Hybrid Input, Zero Offset, 10m Network)
        'S2_SINGLE_TIF': {
        "name": 'S2_SINGLE_TIF', "description": 'Sentinel 2 Custom Single TIF (Zero Offset)',
        "sza": 'SZA', "vza": 'VZA', "saa": 'SAA', "vaa": 'VAA',
        "Collection_SL2P": fc.s2_10m_createFeatureCollection_estimates(),
        "Collection_SL2Perrors": fc.s2_10m_createFeatureCollection_errors(),  
        "sl2pDomain": fc.s2_10m_createFeatureCollection_domains(),
        "Network_Ind": fc.s2_10m_createFeatureCollection_Network_Ind(),
        "numVariables": 7, "exportRes": 20,
        }
    }
...
def make_net_options():
    NET_OPTIONS = {
        'Albedo': {
            ...
            # S2_SINGLE_TIF (7 features, ZERO offset)
            "S2_SINGLE_TIF": {
                "Name": 'Albedo', "description": 'Black sky albedo', "variable": 6,
                "inputBands":      ['cosVZA', 'cosSZA', 'cosRAA', 'B02', 'B03', 'B04', 'B08'], # 7 elements
                "inputScaling":    [1, 1, 1, 0.0001, 0.0001, 0.0001, 0.0001],
                "inputOffset":     [0, 0, 0, 0, 0, 0, 0], # *** CRITICAL CHANGE: ALL ZEROS ***
            },
        },
...
def make_outputParams():
    # output parameters (No changes needed)
    outputParams = {
